function ClusteringPAM(DataSetStartIndex, DataSetEndIndex, DistanceIndex)  
    ...
    for i = 1:length(Datasets)
            ...
            if (i>=DataSetStartIndex & i<=DataSetEndIndex)
                    ...
                    for rep = 1 : 10
                        ...
                        medoids = randsample(DS.DataInstancesCount, length(DS.ClassNames));
                        
                        labels = PartitioningAroundMedoids(medoids,DM);

                        rand_idxs(i) = rand_idxs(i) + RandIndex(labels, DS.DataClassLabels);
                    end
                    rand_idxs(i) = rand_idxs(i) ./ 10;
           
            end
            
            dlmwrite( strcat( 'RESULTS_ClusteringPAM', '_', char(Distances(DistanceIndex)), '_', num2str(DataSetStartIndex), '_', num2str(DataSetEndIndex) ,'.results'), rand_idxs, 'delimiter', '\t');
