function RunOneNNLBKeogh(DataSetStartIndex, DataSetEndIndex, DistanceIndex)
    
    matlabpool close force local;
    matlabpool local 8;
    
    Distances = [cellstr('DTWLBKeogh'), 'cDTWOptimalLBKeogh','cDTW5LBKeogh','cDTW10LBKeogh'];
    ...
    % For cDTW which optimal parameters to use when 1-NN on train set is performed
    ...
    for i = 1:length(Datasets)
            ...
            if (i>=DataSetStartIndex & i<=DataSetEndIndex)
                    ...
                    acc = ParallelOneNNClassifierLBKeogh(DS);
            ...
            dlmwrite( strcat( 'RESULTS_ParallelOneNNClassifierKeogh_', char(Distances(DistanceIndex)) ,'_', num2str(DataSetStartIndex), '_', num2str(DataSetEndIndex) ,'.results'), results, 'delimiter', '\t');
